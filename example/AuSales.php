...
if( isset( $_POST['submit'] ) ){
	require_once( 'com/gmo_pg/client/input/AuSalesInput.php');
	require_once( 'com/gmo_pg/client/tran/AuSales.php');
	
	//入力パラメータクラスをインスタンス化します
	$input = new AuSalesInput();/* @var $input AuSalesInput */
	...
	//API通信クラスをインスタンス化します
	$exe = new AuSales();/* @var $exec AuSales */
	
	//パラメータオブジェクトを引数に、実行メソッドを呼び、結果を受け取ります。
	$output = $exe->exec( $input );/* @var $output AuSalesOutput */
...
//AuSales入力・結果画面
require_once( PGCARD_SAMPLE_BASE . '/display/AuSales.php' );