...
if( isset( $_POST['submit'] ) ){
	require_once( 'com/gmo_pg/client/input/AssignVirtualaccountInput.php');
	require_once( 'com/gmo_pg/client/tran/AssignVirtualaccount.php');

	//入力パラメータクラスをインスタンス化します
	$input = new AssignVirtualaccountInput();/* @var $input AssignVirtualaccountInput */
	...
	//API通信クラスをインスタンス化します
	$exe = new AssignVirtualaccount();/* @var $exec AssignVirtualaccount */

	//パラメータオブジェクトを引数に、実行メソッドを呼び、結果を受け取ります。
	$output = $exe->exec( $input );/* @var $output AssignVirtualaccountOutput */
...
//AssignVirtualaccount入力・結果画面
require_once( PGCARD_SAMPLE_BASE . '/display/AssignVirtualaccount.php' );