...
if( isset( $_POST['submit'] ) ){
	require_once( 'com/gmo_pg/client/input/CvsCancelInput.php');
	require_once( 'com/gmo_pg/client/tran/CvsCancel.php');
	
	//入力パラメータクラスをインスタンス化します
	$input = new CvsCancelInput();/* @var $input CvsCancelInput */
	...
	//API通信クラスをインスタンス化します
	$exe = new CvsCancel();/* @var $exec CvsCancel */
	
	//パラメータオブジェクトを引数に、実行メソッドを呼び、結果を受け取ります。
	$output = $exe->exec( $input );/* @var $output CvsCancelOutput */
...
//CvsCancel入力・結果画面
require_once( PGCARD_SAMPLE_BASE . '/display/CvsCancel.php' );