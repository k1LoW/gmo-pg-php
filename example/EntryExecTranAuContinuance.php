...
if( isset( $_POST['submit'] ) ){
	...
	require_once( 'com/gmo_pg/client/input/EntryExecTranAuContinuanceInput.php');
	require_once( 'com/gmo_pg/client/tran/EntryExecTranAuContinuance.php');

	//入力パラメータクラスをインスタンス化します

	//取引登録時に必要なパラメータ
	$entryInput = new EntryTranAuContinuanceInput();
	...
	$entryInput->setFirstAmount( $_POST['FirstAmount']);
	$entryInput->setFirstTax( $_POST['FirstTax']);

	//決済実行のパラメータ
	$execInput = new ExecTranAuContinuanceInput();
	...
	$execInput->setAccountTimingKbn( mb_convert_encoding( $_POST['AccountTimingKbn'] , 'SJIS' , PGCARD_SAMPLE_ENCODING ) );
	$execInput->setAccountTiming( mb_convert_encoding( $_POST['AccountTiming'] , 'SJIS' , PGCARD_SAMPLE_ENCODING ) );
	$execInput->setFirstAccountDate( mb_convert_encoding( $_POST['FirstAccountDate'] , 'SJIS' , PGCARD_SAMPLE_ENCODING ) );
	...
	$input = new EntryExecTranAuContinuanceInput();/* @var $input EntryExecTranAuContinuanceInput */
	$input->setEntryTranAuContinuanceInput( $entryInput );
	$input->setExecTranAuContinuanceInput( $execInput );

	//API通信クラスをインスタンス化します
	$exe = new EntryExecTranAuContinuance();/* @var $exec EntryExecTranAuContinuance */

	//パラメータオブジェクトを引数に、実行メソッドを呼びます。
	//正常に終了した場合、結果オブジェクトが返るはずです。
	$output = $exe->exec( $input );/* @var $output EntryExecTranAuContinuanceOutput */
...
//EntryExecTranAuContinuance入力・結果画面
require_once( PGCARD_SAMPLE_BASE . '/display/EntryExecTranAuContinuance.php' );