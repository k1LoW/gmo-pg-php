...
if( isset( $_POST['submit'] ) ){
	...
	$input = new EntryTranNetcashInput();/* @var $input EntryTranNetcashInput */
	...
	//API通信クラスをインスタンス化します
	$exe = new EntryTranNetcash();/* @var $exec EntryTranNetcash */

	//パラメータオブジェクトを引数に、実行メソッドを呼び、結果を受け取ります。
	$output = $exe->exec( $input );/* @var $output EntryTranNetcashOutput */
...
//EntryTranNetcash入力・結果画面
require_once( PGCARD_SAMPLE_BASE . '/display/EntryTranNetcash.php' );